...
(function () {
  ...
  YG.init = async (opts = {}) => {
    YG.__initOptions = opts;
    return delay(YG);
  };

  YG.auth = {
    openAuthDialog: async () => {
      if (CFG.fails.auth) return fail("Auth dialog failed");
      return delay({ authorized: true, __mock: true });
    }
  };
  ...
  YG.adv = {
    showFullscreenAdv: async (opts = {}) => {
      if (CFG.fails.advFullscreen) {
        callMaybe(opts.onError, "fullscreen", "mock-fail");
        return fail("Fullscreen ad failed");
      }
      callMaybe(opts.onOpen);
      callMaybe(opts.onOffline);
      await delay(null);
      callMaybe(opts.onClose, true);
      return { shown: true, __mock: true, opts };
    },
    showRewardedVideo: async (opts = {}) => {
      if (CFG.fails.advRewarded) {
        callMaybe(opts.onError, "rewarded", "mock-fail");
        callMaybe(opts.onClose, false);
        return delay({ rewarded: false, __mock: true, reason: "mock-fail" });
      }
      callMaybe(opts.onOpen);
      await delay(null);
      callMaybe(opts.onRewarded);
      callMaybe(opts.onClose, true);
      return { rewarded: true, __mock: true, opts };
    },
    showBannerAdv: async (opts = {}) => {
      if (CFG.fails.banner) {
        callMaybe(opts.onError, "banner", "mock-fail");
        return fail("Banner failed");
      }
      bannerVisible = true;
      callMaybe(opts.onOpen);
      await delay(null);
    ...
    hideBannerAdv: async () => {
      bannerVisible = false;
      return delay({ hidden: true, visible: bannerVisible, __mock: true });
    },
  ...
  const paymentsObj = {
    createPurchase: async (params = {}) => {
      if (CFG.fails.purchase) return fail("Purchase failed");
      const rec = { purchaseToken: "mock-token-" + (purchases.length+1), ...params, __mock: true };
      purchases.push(rec);
      return delay(rec);
    },
    purchase: async (sku, data = {}) => {
      if (CFG.fails.purchase) return fail("Purchase failed");
      const rec = { purchaseToken: "mock-token-" + (purchases.length+1), sku, ...data, __mock: true };
      purchases.push(rec);
      return delay(rec);
    },
    getPurchases: async () => delay({ purchases: purchases.slice(), __mock: true }),
    consumePurchase: async ({ purchaseToken }) => {
      const idx = purchases.findIndex(p => p.purchaseToken === purchaseToken);
      if (idx >= 0) purchases.splice(idx, 1);
      return delay({ consumed: true, purchaseToken, __mock: true });
    },
    getCatalog: async () => {
      if (CFG.fails.catalog) return fail("Catalog unavailable");
      return delay({ products: [], __mock: true });
    }
  };
  ...
  const player = {
    ...
    setLeaderboardScore: async ({ leaderboardName, score, extraData }) => {
      return YG.leaderboards.setLeaderboardScore({ leaderboardName, score, extraData });
    }
  };
  ...
  const storageObj = {
    getItem: async (k) => {
      if (CFG.fails.storage) return fail("Storage read failed");
      return delay(memStore.has(k) ? memStore.get(k) : null);
    },
    setItem: async (k, v) => {
      if (CFG.fails.storage) return fail("Storage write failed");
      memStore.set(k, v);
      return delay(true);
    },
  ...
  const lbObj = {
    setLeaderboardScore: async ({ leaderboardName, score, extraData }) => {
      ...
      if (existing) {
        ...
        list.push({ uid: MOCK_UID, name: MOCK_NAME, photo: MOCK_PHOTO, score: Number(score) || 0, extraData });
      }
      list.sort((a,b) => b.score - a.score);
      leaderboards.set(leaderboardName, list);
      return delay({ ok: true, leaderboardName, score: Number(score)||0, extraData, __mock: true });
    },
    getLeaderboardEntries: async ({ leaderboardName, quantityTop = 10, quantityAround, includeUser } = {}) => {
      ...
      let userRank = list.findIndex(e => e.uid === MOCK_UID);
    ...
    getLeaderboardDescription: async ({ leaderboardName }) =>
      delay({ name: leaderboardName, description: "", __mock: true }),
    getLeaderboardPlayerEntry: async ({ leaderboardName }) => {
      const list = (leaderboards.get(leaderboardName) || []).slice();
      const idx = list.findIndex(e => e.uid === MOCK_UID);
  ...
  YG.features = {
    Feedback: {
      canReview: async () => {
        if (CFG.fails.feedback) return fail("Feedback not available");
        return delay({ value: false, reason: "unsupported", __mock: true });
      },
      requestReview: async () => {
        if (CFG.fails.feedback) return fail("Feedback request failed");
        return delay({ feedbackSent: false, __mock: true });
      }
    },
    Shortcuts: {
      canShowPrompt: async () => {
  ...
  YG.clipboard = {
    writeText: async (text) => delay(true)
  };

  const flagsObj = {
    _map: {},
    get(key, fallback = undefined) { return Object.prototype.hasOwnProperty.call(this._map, key) ? this._map[key] : fallback; },
    has(key) { return Object.prototype.hasOwnProperty.call(this._map, key); },
    toJSON() { return { ...this._map }; },
    _set(key, value) { this._map[key] = value; return true; },
    _clear() { this._map = {}; return true; }
  };
  YG.getFlags = async () => flagsObj;
  YG.flags = flagsObj;
  ...
  const _events = {};
  YGL.on = (evt, fn) => { (_events[evt] || (_events[evt]=[])).push(fn); };
  YGL.off = (evt, fn) => { const a=_events[evt]; if(!a) return false; const i=a.indexOf(fn); if(i>=0) a.splice(i,1); return i>=0; };
